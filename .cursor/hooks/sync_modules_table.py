...
# ── self-test ───────────────────────────────────────────────────────
def self_test() -> int:
    """python3 .cursor/hooks/sync_modules_table.py --self-test

    验证 5 项：
      1. find_all_marker_blocks：v3 / v2 / v1 三协议命中
      2. find_marker_block：按 block_id 过滤
      3. build_synced_block：构造的区块含 metadata + banner
      4. inject_block_into_content：注入新区块（替换 / 追加两条路径）
      5. main() 空 stdin / 无效 JSON / 文件不存在 → 不崩
    """
    # Case 1: find_all_marker_blocks 三协议
    sample_v3 = '''前置文本
<aside data-modules-sync-block="§1" data-src="x.md" data-sha256="abc">content §1</aside>
中间
<aside data-modules-sync-block="§3.5" data-src="y.md">content §3.5</aside>
尾部'''
    found_v3 = find_all_marker_blocks(sample_v3)
    assert len(found_v3) == 2, f"Case 1a: expected 2 blocks (v3), got {len(found_v3)}"
    assert found_v3[0][0] == "§1", f"Case 1a: expected '§1', got '{found_v3[0][0]}'"
    assert found_v3[1][0] == "§3.5", f"Case 1a: expected '§3.5', got '{found_v3[1][0]}'"

    sample_v2 = '''前置
<!-- BEGIN: MODULES_SYNC id=§1 src=x sha256=abc -->v2 block<!-- END: MODULES_SYNC id=§1 -->
中'''
    found_v2 = find_all_marker_blocks(sample_v2)
    assert len(found_v2) == 1, f"Case 1b: expected 1 block (v2), got {len(found_v2)}"
    assert found_v2[0][0] == "§1", f"Case 1b: expected '§1', got '{found_v2[0][0]}'"

    sample_v1 = '''前
<!-- BEGIN: MODULES_TABLE_SYNC -->
v1 block content
<!-- END: MODULES_TABLE_SYNC -->
后'''
    found_v1 = find_all_marker_blocks(sample_v1)
    assert len(found_v1) == 1, f"Case 1c: expected 1 block (v1), got {len(found_v1)}"
    assert found_v1[0][0] == "§1", f"Case 1c: v1 应映射到 §1, got '{found_v1[0][0]}'"
    print(f"  [OK] Case 1: find_all_marker_blocks 三协议（v3 2 块 / v2 1 块 / v1→§1 1 块）")

    # Case 2: find_marker_block 按 block_id 过滤
    found_1 = find_marker_block(sample_v3, "§1")
    assert found_1 is not None, "Case 2: §1 should exist"
    found_35 = find_marker_block(sample_v3, "§3.5")
    assert found_35 is not None, "Case 2: §3.5 should exist"
    found_none = find_marker_block(sample_v3, "§4.11.2")
    assert found_none is None, "Case 2: §4.11.2 should not exist"
    print(f"  [OK] Case 2: find_marker_block 按 id 过滤（§1 / §3.5 命中 / §4.11.2 None）")

    # Case 3: build_synced_block 构造区块
    block = build_synced_block("§1", "## 8 模块总表\n内容", "deadbeef1234567890abcdef")
    assert 'data-modules-sync-block="§1"' in block, "Case 3: 应含 data-modules-sync-block"
    assert 'data-src=".cursor/MODULES.md#§1"' in block, "Case 3: 应含 data-src"
    assert 'data-sha256="deadbeef1234567890abcdef"' in block, "Case 3: 应含 data-sha256"
    assert "🔴" in block, "Case 3: 应含红色 banner"
    assert "</aside>" in block, "Case 3: 应含 </aside>"
    assert "## 8 模块总表" in block, "Case 3: 应含 injected 内容"
    print(f"  [OK] Case 3: build_synced_block 5 项 metadata + banner")

    # Case 4: inject_block_into_content 替换路径
    existing = '''前置
<aside data-modules-sync-block="§1">old</aside>
尾部'''
    new_block = '<aside data-modules-sync-block="§1">new</aside>'
    after = inject_block_into_content(existing, "§1", new_block)
    assert "old" not in after, "Case 4a: 旧内容应被替换"
    assert "new" in after, "Case 4a: 新内容应在"
    assert "前置" in after and "尾部" in after, "Case 4a: 前后文本保留"

    # Case 4b: append 路径
    empty_tail = "纯文本内容"
    appended = inject_block_into_content(empty_tail, "§3.5", new_block)
    assert appended.endswith(new_block + "\n"), "Case 4b: 应追加到末尾"
    print(f"  [OK] Case 4: inject_block_into_content replace + append")

    # Case 5: main() 边界
    proc5a = __import__("subprocess").run(
        [sys.executable, __file__], input="",
        capture_output=True, text=True, timeout=10,
    )
    assert proc5a.returncode == 0, f"Case 5a (空 stdin): expected 0, got {proc5a.returncode} stderr={proc5a.stderr}"
    proc5b = __import__("subprocess").run(
        [sys.executable, __file__], input="not-json",
        capture_output=True, text=True, timeout=10,
    )
    assert proc5b.returncode == 0, f"Case 5b (无效 JSON): expected 0, got {proc5b.returncode}"
    proc5c = __import__("subprocess").run(
        [sys.executable, __file__],
        input=json.dumps({"file_path": "/tmp/nonexistent_zzz_12345.md"}),
        capture_output=True, text=True, timeout=10,
    )
    assert proc5c.returncode == 0, f"Case 5c (文件不存在): expected 0, got {proc5c.returncode}"
    print(f"  [OK] Case 5: main() 空 stdin / 无效 JSON / 文件不存在均 exit 0")
