...
| 文件 | 路径 |
|------|------|
| 项目配置 | `workflow_assets/<req_name>/<version>/「S2.5 迭代规划」/project_config.json` |
| 迭代规划报告 | `workflow_assets/<req_name>/<version>/「S2.5 迭代规划」/iteration_plan.md` |
| 迭代规划数据 | `workflow_assets/<req_name>/<version>/「S2.5 迭代规划」/iteration_plan.json` |
...
路径：`workflow_assets/<req_name>/<version>/「S2.5 迭代规划」/fail_report_S2_5.md`
...
<aside data-exec-card-block="input_gate" data-src=".cursor/rules/STAGE_S2_5_ITERATION.mdc" data-sha256="INIT_seed" data-synced-at="2026-07-14">

> ⚠️ **派生产物，禁止直接修改** — 本块由 `scripts/sync_execution_cards.py` 自动生成
> src: `.cursor/rules/STAGE_S2_5_ITERATION.mdc` | synced_at: `2026-07-14`
> 修改请改源文件，然后跑 `python3 scripts/sync_execution_cards.py --stage s2-5-iteration` 重新同步。

### 输入门禁（input_gate）

| 必备材料 | 路径 | 缺失处理 |
|---|---|---|
| S2 backlog.md | `workflow_assets/<req_name>/<version>/「S2 需求拆解」/backlog.md` | 生成 fail_report_S2_5.md，停止 |
| S2 requirement_objects.json | `workflow_assets/<req_name>/<version>/「S2 需求拆解」/requirement_objects.json` | 工时估算依据 |
| project_config.json | `workflow_assets/<req_name>/<version>/「S2.5 迭代规划」/project_config.json` | 配置不全需询问用户，禁止默认 |

**触发命令**：`/aidocx-s2-5-iteration` 或粘贴 S2 backlog；⚠️ 全流程默认跳过（opt-in）

</aside>

<aside data-exec-card-block="field_required" data-src=".cursor/rules/STAGE_S2_5_ITERATION.mdc" data-sha256="INIT_seed" data-synced-at="2026-07-14">

### 必填字段（field_required）

| 字段 | 级别 | 校验 |
|---|---|---|
| `project_config.json` 全部字段 | **MUST** | req_name / version / 排期日期 / 各角色工时 / 团队规模 |
| 7 步规划输出 | **MUST** | Story 优先级排序 / 里程碑 / 风险点 |
| `iteration_plan.json` | **MUST** | 最终规划文档 |

</aside>

<aside data-exec-card-block="quality_gate" data-src=".cursor/rules/STAGE_S2_5_ITERATION.mdc" data-sha256="INIT_seed" data-synced-at="2026-07-14">

### 质量门禁（quality_gate）

| 门禁 | 阈值 | 说明 |
|---|---|---|
| 项目配置完整率 | 100% | 配置不全必须询问用户，禁止默认 |
| 里程碑合理性 | 工时累计 ≤ 迭代总工时 × 1.1 | 允许 ±10% 弹性 |
| 负载均衡 | 各角色工时不超过上限 120% | 防止过载 |

</aside>

<aside data-exec-card-block="naming" data-src=".cursor/rules/STAGE_S2_5_ITERATION.mdc" data-sha256="INIT_seed" data-synced-at="2026-07-14">
...
| 迭代规划 | `iteration_plan.md` / `iteration_plan.json` |
| 输出目录 | `workflow_assets/<req_name>/<version>/「S2.5 迭代规划」/` |
