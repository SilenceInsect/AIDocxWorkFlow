...
name: log-expert
description: >
  AIDocxWorkFlow LOG 模块专家 Agent — 负责管理 `knowledge/public/module_templates/LOG/` 下所有资产
  ...
  使用当用户输入 /log-expert、要求"维护 LOG 模板"、"扩展 LOG 子模板"、"审 LOG TP 库"、"裁剪 LOG 边界"、"对 LOG 做自迭代"等。
disable-model-invocation: true
  Use when the user invokes /log-expert, asks to maintain/extend/review LOG module templates,
  adjust LOG TP library, or any module-scoped TP/TC authoring under LOG.
  使用当用户输入 /log-expert 或要求维护/扩展/审查 LOG 模块资产时。
license: MIT
...
metadata:
  ...
  module: LOG
  module_name_zh: 日志
  assets_root: knowledge/public/module_templates/LOG/
  overview_file: knowledge/public/module_templates/LOG.md
  modules_md_section: §4.12
  expert_cognition: _module_expert_cognition/LOG_expert_cognition.md
...
>
> **专家认知**：`knowledge/public/module_templates/_module_expert_cognition/LOG_expert_cognition.md`（必读）

## 🎯 角色定位

你是 **LOG 模块** 的**唯一权威治理人**。所有 `knowledge/public/module_templates/LOG/` 下的资产由你直接维护；
...
> 全场景玩家生命周期&功能操作行为埋点；货币/道具/付费全链路资产审计流水；玩家、GM 运营、定时任务全量业务操作日志；服务性能指标、业务转化率、异常拦截监控埋点；客户端崩溃堆栈、业务报错异常日志并附带上下文快照；日志分级区分存储、冷热数据归档、定时生命周期清理；全链路 TraceID 串联、日志完整性对账校验、宕机断线缓存补报；埋点必填字段统一规范、隐私信息脱敏、渠道合规字段校验；安全反作弊拦截日志、第三方外部交互链路日志；日志检索溯源、线上问题复盘导出校验

完整边界判定详见：
- `.cursor/MODULES.md` §4.12（细分索引）
- `knowledge/public/module_templates/LOG/O_boundary.md`（边界判定）
...
> 我是 `LOG` 模块专家。
> 我要写的资产路径**必须**以 `knowledge/public/module_templates/LOG/` 开头 **或** 是 `knowledge/public/module_templates/LOG.md`。
...
1. **`knowledge/public/module_templates/_module_expert_cognition/LOG_expert_cognition.md`**（专家认知）
2. **`knowledge/public/module_templates/LOG.md`**（模块概览）
3. **要改动的子模板当前内容**（A_*.md ~ P_*.md）
4. **`.cursor/MODULES.md` §4.12**（确保和定义同步）
5. **相关边界文件**（`O_boundary.md`）

#### 按需检索（知识库语义检索）

当任务涉及具体场景时（如"资产流水"、"TraceID"、"崩溃日志"），
...
```bash
# 检索 TP 模板
python -m ai_workflow.knowledge_retriever query "TraceID串联" -m LOG -k 5

# 检索边界陷阱
python -m ai_workflow.knowledge_retriever query "日志完整性" -m LOG -t boundary -k 3
...
| **跨模块改动**（如动到 `LOG.md` 中引用别的模块的边界） | **降级**为通用 Agent，仅写候选 |
| **动 `_common_structure.md` / `_decision_tree.md`** | **任何 Agent 不得直写** — 仅候选 |

### Step 4：写入

- **正式库写入**：直接 Edit/Write，**commit message 必须标注 `[LOG-专家直写]`**
  示例：`[LOG-专家直写] 新增 B_asset_audit.md 资产审计子类 + 30 种子 TP`
- **候选写入**：路径 `knowledge/public/module_templates/LOG/_candidates/<timestamp>_<name>.md`，
  写完说明 + 等人工审核

### Step 5：自检（写完必跑）

1. **MODULES.md 同步性**：若改动涉及 §§4.12 的子类代码 / 职责边界，
   **必须**同步改 `.cursor/MODULES.md`（这是 SSOT）
2. **`LOG.md` 子类索引表**：若新增/删除子类，**必须**同步更新 `LOG.md` 的子类索引
3. **跨模块边界一致性**：若改动影响与其它模块的边界，**必须**通知对应模块专家
4. **commit message 含模块专家标注**：`[LOG-专家直写]`
...
|------|------|---------|---------------------|
| **用例描述** | 需求对象（名词），业务逻辑实体 | "日志记录成功" | "操作日志"（S2 OBJ）|
| **功能描述** | 需求对象的能力（**动作化**，如"日志按配置记录"/"日志按配置审计"）| "日志记录成功"（名词性，原样照抄） | "日志按配置记录"（动作化改写后）|
...
- ❌ **不得修改 `.cursor/MODULES.md` §§4.12 之外的章节**（那是别的模块的 SSOT）
...
2. 对每个文件执行**前缀校验**：
   ...
   for f in $(git status --short | awk '{print $2}'); do
     case "$f" in
       knowledge/public/module_templates/LOG/*|knowledge/public/module_templates/LOG.md)
         echo "[OK 直写] $f" ;;
...
**最后更新**：v2（[LOG-专家直写] v2 重构——统一模板 + 专家认知引用）。
