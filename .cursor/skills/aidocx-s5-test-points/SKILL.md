...
> **触发条件**：当执行 `/aidocx-s5-test-points` 且 `project_name` 参数存在（或显式指定"模块编排模式"）时，启用本模式。
> **不改变默认行为**：单 agent 直接生成 TP 的方式仍然保留，本节是**额外编排路径**。

### §1 模式切换

| 模式 | 触发 | 主 agent 角色 |
|------|------|--------------|
| **默认模式**（直接生成） | 无 `project_name` | 直接生成 TP 写入 test_points.json |
| **模块编排模式**（v35 新增）| 有 `project_name` | orchestrator：调度 8 expert + merge-expert |

### §2 流水线

主 agent（orchestrator，不自己写 TP）
  ↓ orchestrator prompt（由 conversation_skills 生成）
  ├─ Task(/ui-expert)   → _module_expert_drafts/UI_module_tp.json
  ├─ Task(/biz-expert)  → _module_expert_drafts/BIZ_module_tp.json
  ├─ Task(/config-expert) → _module_expert_drafts/CONFIG_module_tp.json
  ├─ Task(/util-expert)  → _module_expert_drafts/UTIL_module_tp.json
  ├─ Task(/link-expert) → _module_expert_drafts/LINK_module_tp.json
  ├─ Task(/log-expert)  → _module_expert_drafts/LOG_module_tp.json
  ├─ Task(/special-expert) → _module_expert_drafts/SPECIAL_module_tp.json
  └─ Task(/hint-expert) → _module_expert_drafts/HINT_module_tp.json
  ↓（全部完成）
  /merge-expert → test_points.json
  ↓
主 agent 调用 save_stage5_output()

### §3 草稿格式

每个 expert 写入 `<stage_dir>/_module_expert_drafts/<MODULE>_module_tp.json`：
...
```json
{
  "meta": {
    ...
    "req_name": "游戏道具商城系统",
    "version": "v3.01",
    "created_at": "2026-07-22T09:22:00+08:00",
    "story_ids": ["UI-001-001", "UI-001-002"]
  },
  "test_points": [...]
}
```
...
| 方面 | 默认模式 | 模块编排模式 |
|------|----------|--------------|
| 主 agent 产出 | 自己写 TP | 只做编排，不写 TP |
| 并行性 | 无 | 8 expert 并行（≤5 批）|
| 合并 | 无 | merge-expert 串行收尾 |
| 上下文长度 | 所有 TP 在上下文中 | 上下文只含 orchestrator prompt |
