...
| # | 材料 | 路径 | 必读原因 |
|---|---|---|---|
| 1 | 8 模块总表 | `.cursor/MODULES.md`（§1 总表）| 终版需求.md 里的 Story/OBJ 必须有模块前缀 |
| 2 | S1 终版需求（草稿）| `workflow_assets/<req_name>/「S1 需求评审」/<version>/终版需求.md` | 所有完善工作的起点 |
| 3 | clarification_checklist | `workflow_assets/<req_name>/「S1 需求评审」/<version>/clarification_checklist.md` | 人工填写的处理方案是完善依据 |
| 4 | review_report | `workflow_assets/<req_name>/「S1 需求评审」/<version>/review_report.md` | S1 评审结论和评分是质量基准 |
