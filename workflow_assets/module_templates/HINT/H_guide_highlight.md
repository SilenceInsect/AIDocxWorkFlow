...
> **不测什么**：引导 UI 容器样式（归 UI `F.GUIDE_HINT`）、引导底层框架（步骤管理器）→ AUX、新手业务奖励逻辑（归 BIZ `A.BIZ_LOGIC`）。
...
### 边界 3：vs AUX `AUX.GUIDE_FRAMEWORK`
- **混淆点**：「引导"框架"」——H 测引导本身、AUX 测底层框架
- **判定规则**：测"具体引导内容" → H；测"步骤管理器底层" → AUX
- **instance**：新手第 3 步内容 → H；步骤管理器状态机 → AUX
