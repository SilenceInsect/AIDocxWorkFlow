...
> **不测什么**：通用日志工具（归 AUX）、业务逻辑（归 A）、状态机（归 D）、付费业务（归 H）。
> **与其他子类的差异**：I 关注"业务审计日志"——AUX J 关注"通用日志工具"，LOG 模块关注"通用行为日志"，H 关注"付费业务"。
...
### 边界 1：vs AUX J. 存储+日志
- **混淆点**：「业务"日志"」——I 测业务、AUX J 测通用
- **判定规则**：测"通用日志工具/采集" → AUX J；测"业务审计日志" → I
- **实例**：日志 SDK → AUX J；业务操作审计 → I
...
### 边界 5：vs AUX G. GM 工具
- **混淆点**：「GM"操作"」——AUX G 测 GM、I 测审计
- **判定规则**：测"GM 指令" → AUX G；测"GM 操作审计" → I
- **实例**：GM 指令工具 → AUX G；GM 操作审计日志 → I
