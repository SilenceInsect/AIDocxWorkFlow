...
#   5. Print a one-page summary of what's installed and what's next.
#
# Usage:
#   ./install.sh            # full run
...
# ── 3. Hermes install ────────────────────────────────────────────────────
step "3/5 Hermes skill: aidocx-batch-runner"
HERMES_SKILL_SRC="workflow_assets/hermes_skills/aidocx-batch-runner"
if [ ! -d "$HERMES_SKILL_SRC" ]; then
  fail "Source directory $HERMES_SKILL_SRC missing. Did you clone the repo fully?"
  exit 1
fi
...
# ── 4. validate SKILL.md ────────────────────────────────────────────────
step "4/5 Validating SKILL.md (agentskills.io 1.0)"
...
${BOLD}Installed:${RESET}
  ...
  • 1 hermes skill: aidocx-batch-runner
  • 1 Cursor hook: aidocx_feedback_logger_hook.py
    (registered on beforeSubmitPrompt + sessionEnd)
...
${BOLD}Next steps:${RESET}
  ...
  4. (Optional) For the batch-runner: see workflow_assets/hermes_skills/.
