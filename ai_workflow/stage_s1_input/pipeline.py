...
Directory layout:
  workflow_assets/<req_name>/v1.0/「S1 需求评审」/
    raw/
      extracted_text.md
      extracted_images/
...
def run_s1_pipeline(
    ...
    Args:
        ...
        version: Version string. Must match the directory version being worked on
            (e.g. "v3.01"). Defaults to "v1.0" for backward compatibility but
            callers should pass the correct version explicitly.
    ...
    # v8 实战决策：S1 子模块入口也走 purge 询问（覆盖直接调用 run_s1_pipeline 的场景）
    from ai_workflow.conversation_skills import _prompt_purge_existing
    decision = _prompt_purge_existing("S1", req_name, version)
    if decision["action"] == "keep":
        return PipelineResult(
            req_name=req_name,
            success=False,
            output_dir=pipeline.output_root,
            docx_path=Path(docx_path) if docx_path else None,
            error_message=f"S1 skipped: {decision.get('reason', 'user_chose_keep_existing')}",
        )
