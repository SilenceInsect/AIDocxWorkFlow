...
def _check_type_a_materials(
    ...
    for sm in skill_materials:
        ...
        if not found:
            ...
            # 提取材料名中的关键标识（如 "workflow_assets/module_templates/"）
            has_keyword = False
            for rm in rule_materials:
...
def run_consistency_check(stage: str) -> dict[str, Any]:
    """
    对指定阶段执行一致性检查。
    ...
    """
    # 检查缓存
    if stage in _CHECK_CACHE:
        result = _CHECK_CACHE[stage].copy()
    ...
    if not skill_text:
        ...
        _CHECK_CACHE[stage] = result.copy()
        return result
