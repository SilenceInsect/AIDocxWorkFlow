#!/usr/bin/env python3
"""S1 — 材料门禁（纯检查，非评分）+ S1 评分辅助 + 优化识别（S1.8）+ 评审产物生成（v18 重构）。
...
5. generate_review_issues() — 生成 review_issues.md 结构框架（v18 新增）
6. generate_edge_cases() — 生成 edge_cases.md 结构框架（v18 新增）
7. generate_testability_assessment() — 生成 testability_assessment.md 结构框架（v18 新增）
...
def detect_optimization_blocks(text: str) -> OptimizationManifest:
    ...
    section_blocks: list[tuple[int, str]] = []  # (line_no, title)
...
def auto_review_requirement(requirement_text: str) -> dict:
    ...
    return {
        ...
        "review_framework": {
            ...
            "output_dir": "workflow_assets/<req_name>/<version>/「S1 需求评审」/",
        },
...
# ─────────────────────────────────────────────────────────────────────────────
# S1 v18 — 评审产物生成（3 个新增产出）
# ─────────────────────────────────────────────────────────────────────────────

def generate_review_issues(
    req_name: str,
    version: str,
    reviewer: str = "AIDocxWorkFlow",
) -> dict:
    """
    生成 review_issues.md 的结构化框架，供 LLM 填充。

    v18 新增产物（核心硬性产出 #1）。

    返回 dict 包含：
      - "path": 输出文件路径（相对路径）
      - "template": Markdown 模板字符串
      - "sections": 分节说明
    """
    date_str = __import__("datetime").datetime.now().strftime("%Y-%m-%d")
    template = f"""# 需求评审问题记录表
...
|| # | 角色 | 问题描述 | 影响范围 | 优先级 |
|---|-----|------|----------|--------|
|| 1 | [角色名] | [问题描述] | [影响模块] | P0/P1/P2 |

## 2. 需求对象问题

|| # | 需求对象 | 问题描述 | 关联角色 | 优先级 |
|---|-----|------|----------|--------|
|| 1 | [对象名] | [问题描述] | [角色] | P0/P1/P2 |

## 3. 功能/数据流问题

|| # | 功能 | 问题描述 | 数据流节点 | 优先级 |
|---|-----|------|----------|--------|
|| 1 | [功能名] | [问题描述] | [入口/处理/出口] | P0/P1/P2 |
...
"""
    return {
        "path": f"workflow_assets/{req_name}/{version}/「S1 需求评审」/review_issues.md",
        "template": template,
        "sections": {
            "1_角色同步问题": "按主/次/边界角色分类，记录 AI 在评审中发现的问题",
            "2_需求对象问题": "按需求对象（商城下单/发奖系统等）分类，记录结构性问题",
            "3_功能数据流问题": "按功能及其数据流节点（入口/处理/出口）分类",
            "4_问题汇总": "汇总各优先级的问题数量",
        },
        "note": "LLM 填充时，每个问题应包含：问题描述（清晰）+ 影响范围（模块/角色）+ 优先级（P0/P1/P2）",
    }
...
def generate_edge_cases(
    req_name: str,
    version: str,
    author: str = "AIDocxWorkFlow",
) -> dict:
    """
    生成 edge_cases.md 的结构化框架，供 LLM 填充。
...
|| # | 场景名称 | 异常类型 | 预期行为 | 测试关注点 |
|---|-----|------|----------|----------|
|| 1 | [异常场景] | [类型] | [预期] | [测试点] |

## 3. 并发/竞态场景

|| # | 场景名称 | 并发条件 | 预期行为 | 测试关注点 |
|---|-----|------|----------|----------|
|| 1 | [并发场景] | [条件] | [预期] | [测试点] |

## 4. 数据边界场景

|| # | 场景名称 | 数据边界 | 预期行为 | 测试关注点 |
|---|-----|------|----------|----------|
|| 1 | [数据边界] | [边界值] | [预期] | [测试点] |
"""
    return {
        "path": f"workflow_assets/{req_name}/{version}/「S1 需求评审」/edge_cases.md",
        "template": template,
        "sections": {
            "1_边界条件场景": "正常操作的上限/下限/临界点（如：最大购买数量、余额为0、时间戳边界）",
            "2_异常流程场景": "操作失败/系统异常时的行为（如：支付失败、回调超时、接口报错）",
            "3_并发竞态场景": "多用户/多线程/异步任务竞态条件（如：重复领取、库存超卖）",
            "4_数据边界场景": "数据类型边界（如：负数、空值、特殊字符、超长字符串）",
        },
        "note": (
            "LLM 填充时，每个场景应包含：触发条件（精确）+ 预期行为（明确）+ 测试关注点（可操作）。"
            "与 testability_assessment.md 的测试盲区互补——边界场景侧重'已知风险'，盲区侧重'未知风险'。"
        ),
    }
...
def generate_testability_assessment(
    req_name: str,
    version: str,
    assessor: str = "AIDocxWorkFlow",
) -> dict:
    """
    生成 testability_assessment.md 的结构化框架，供 LLM 填充。
...
"""
    return {
        "path": f"workflow_assets/{req_name}/{version}/「S1 需求评审」/testability_assessment.md",
        "template": template,
        "sections": {
            "1_可测性总评": "4 个维度评分（可观测性/可控制性/可自动化程度/测试覆盖完整性）",
            "2_测试盲区识别": "AI 识别到的测试难以覆盖的区域（🔴高/🟡中/🟢低风险）",
            "3_测试数据依赖": "测试所需的种子数据是否在需求中明确（✅/❌）",
            "4_测试环境依赖": "测试环境/工具/权限是否就绪（✅/❌/⚠️）",
        },
        "note": (
            "可测性评估与 5 维度评分的关系："
            "可测性维度侧重'能测吗'（可观测/可控制/可自动化），"
            "5 维度可测试性侧重'测得准吗'（业务故事有明确通过/失败条件）。"
            "两者互补，共同构成 S1 可测试性评审的完整视图。"
        ),
    }
