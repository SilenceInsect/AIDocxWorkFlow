...
| 3 | S2 backlog | `workflow_assets/<req_name>/「S2 需求拆解」/<version>/backlog.md` + `backlog.json` | Epic/Story 是 TP 的核心来源 |
| 4 | S4 business_flow | `workflow_assets/<req_name>/「S4 流程图导出」/<version>/business_flow.md` | **EXCEPTION 类型 TP 的核心来源**（风险点 + 异常树叶子） |
| 5 | 模块子模板 | `workflow_assets/module_templates/<MODULE>/<X>_*.md`（按命中模块按需） | 种子 TP 模板，按子类展开 |
| 6 | 通用 5 段结构 | `workflow_assets/module_templates/_common_structure.md` | TP 格式规范 |
...
> 完整边界规则 + 决策树见 `workflow_assets/module_templates/HINT/O_boundary.md`。
> **核心判定三问**：
...
S5 评审期间必须产出以下文件（落到 `workflow_assets/<req_name>/「S5 测试点生成」/<version>/`）：
