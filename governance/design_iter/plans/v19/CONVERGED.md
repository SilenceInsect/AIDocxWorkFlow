...
**执行轮次**: 4 次 audit() 调用（R1/R2/R3/R4），有效审计 3 轮（R1+R2+R3 + R4 为终态合并）
...
1. **`ai_workflow/goal_snapshot.py`** — v2（427行）
   - `GOALS_DIR = REPO_ROOT / ".goal-log-db" / "active"`
   - `INDEX_DIR = REPO_ROOT / ".goal-log-db" / "index"`
   - 新增 `_append_session_index()` 和 `_update_thread_goals()` 函数
   - `create_snapshot()` 和 `update_snapshot()` 自动维护索引
   - `self_test()` 升级到 10 cases

2. **`ai_workflow/goal_loop_runner.py`** — v2（510行）
   - 文档字符串更新为新路径 `.goal-log-db/active/<goal_id>/`
...
| `goal_loop_runner.py --self-test` | **10/10 PASS** |
| `python3 -m py_compile goal_snapshot.py` | ✅ |
| `python3 -m py_compile goal_loop_runner.py` | ✅ |
