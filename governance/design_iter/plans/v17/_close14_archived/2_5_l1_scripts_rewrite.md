...
## 4. 后续批次（s6_report.py + check_field_completion.py）

**s6_report.py**：S6 报告生成器。根据 v17 PLAN §2.2 必改清单，需移除"v2 锚点报告"逻辑（如果有）。建议读全文后做最小改动（注释清理 + 移除锚点相关报告项）。
