...
- **8 模块体系**：覆盖游戏/工具类产品的全部测试场景（CONFIG / UI / BIZ / AUX / LINK / SPECIAL / LOG / HINT）
...
| **AUX** 辅助 | `AUX-` | 底层工具框架、网络层、缓存层、资源管理、离线更新、GM 工具、性能监控、加密安全 |
...
│
├── .cursor/                        # Cursor 配置（规则 / skills / hooks）
...
│   ├── hooks/                      # Cursor 钩子
│   │   ├── project_dna_inject.py           # sessionStart：DNA 二次注入
│   │   ├── before_prompt_dna_check.py      # beforeSubmitPrompt：3 问自检前置
│   │   ├── dna_violation_check.py          # DNA 违规检查
│   │   ├── dna_decision_density_check.py   # 决策密度检查
...
│       └── md_converter.py
│
├── workflow_assets/                 # 全部中间产物（不纳入版本控制）
│   ├── feedback_logs/               # 每轮会话反馈日志
│   ├── feedback_archive/             # AI 经验知识库（按 4 分类归档）
│   │   ├── 业务/                    # 某类业务的常见漏测点
│   │   ├── 方法论/                  # 测试方法论层面的改进
│   │   ├── 界面/                   # UI 测试补充点
│   │   └── 配置/                   # 配置类需求补充点
│   ├── module_templates/             # 8 模块测试点模板（S5 准入物料）
│   │   ├── UI/                      # UI 模块 10 子类模板
│   │   ├── BIZ/                     # BIZ 模块 11 子类模板
...
4. **经验归档** — 把可复用的教训写进 `feedback_archive/`，分四类：
   - `业务/` — 某类业务的常见漏测点
   - `方法论/` — 测试方法论层面的改进
   - `界面/` — UI 测试补充点
   - `配置/` — 配置类需求补充点
...
CONFIG（配置）/ UI（界面）/ BIZ（业务）/ AUX（辅助）/ LINK（关联）/ SPECIAL（特殊情境）/ LOG（日志）/ HINT（提示）。详见 [`.cursor/MODULES.md`](.cursor/MODULES.md)。
</details>
