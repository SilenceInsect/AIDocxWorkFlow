...
- `compose_test_points_from_structure()`（行 22-106）：从 backlog 生成测试点骨架，仅搬运 Story 原始字段，`scenario_test_points: []` 完全由 LLM 按 SKILL.md §1.4 推理填入。
...
| story.acceptance_criteria | ✓ 被 `compose_test_points_from_structure()` 搬运到骨架 |
| story.precondition / input_data / expected_output | ✓ 被 `compose_test_points_from_structure()` 搬运到骨架 |
