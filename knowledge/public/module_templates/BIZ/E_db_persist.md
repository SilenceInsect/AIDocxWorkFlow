...
> **不测什么**：业务逻辑（归 A）、缓存层（归 AUX C）、分布式中间件（归 AUX）。
...
### 边界 3：vs AUX C. 缓存
- **混淆点**：「数据"缓存"」——AUX C 测缓存、E 测 DB
- **判定规则**：测"Redis 缓存" → AUX C；测"DB 落库" → E
- **实例**：Redis 缓存一致性 → AUX C；DB 事务锁 → E
...
### 边界 5：vs AUX N. 异常兜底
- **混淆点**：「DB"异常"」——AUX N 测通用异常、E 测 DB
- **判定规则**：测"通用异常兜底" → AUX N；测"DB 特定异常" → E
- **实例**：全局崩溃 → AUX N；DB 抖动重试 → E
