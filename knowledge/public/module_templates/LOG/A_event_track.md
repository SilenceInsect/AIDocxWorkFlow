...
> **不测什么**：日志底层采集 SDK（归 UTIL）、日志存储/上报（归 F）、埋点字段合规（归 H）、埋点链路追踪（归 I）、业务审计对账（归 BIZ-I）。
...
### 边界 4：vs UTIL（埋点 SDK）
- **混淆点**：「埋点"上报"」——A 测业务触发规则、UTIL 测 SDK
- **判定规则**：测"业务应该埋什么点" → A；测"SDK 怎么采集/上报" → UTIL
- **实例**：抽卡埋点触发 → A；埋点 SDK 断网缓存 → UTIL
