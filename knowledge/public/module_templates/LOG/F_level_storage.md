...
> **不测什么**：业务日志内容（归 A/B/C）、日志底层采集 SDK（归 UTIL）、本地存档（归 UTIL J）、数据库存储（归 BIZ E）。
> **与其他子类的差异**：F 关注"日志**存储/生命周期/容灾**规范"——A 关注"行为埋点"，B 关注"资产对账"，C 关注"操作日志"。与 UTIL 严格隔离：UTIL 测"底层存储技术能力"，F 测"日志业务分层/分级/生命周期规范"。
...
### 边界 1：vs UTIL（本地存储）
- **混淆点**：「本地"存储"」——F 测日志业务分层、UTIL J 测底层
- **判定规则**：测"日志业务分级/生命周期" → F；测"通用 localStorage 工具" → UTIL J
- **实例**：DEBUG 自动清理 → F；玩家设置存档 → UTIL J
