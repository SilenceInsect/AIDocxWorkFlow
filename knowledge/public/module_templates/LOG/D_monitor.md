...
> **不测什么**：业务逻辑（归 BIZ）、底层监控工具（归 UTIL K）、服务性能压测（归 SPECIAL）、业务操作日志（归 C）、资产流水（归 B）。
...
### 边界 4：vs UTIL K. 性能工具
- **混淆点**：「性能"埋点"」——D 测业务指标、UTIL K 测工具
- **判定规则**：测"业务性能指标" → D；测"通用性能监控组件" → UTIL K
- **实例**：接口 P99 业务埋点 → D；FPS 监控组件 → UTIL K
