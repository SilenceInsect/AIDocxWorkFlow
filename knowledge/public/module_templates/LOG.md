...
| O | 边界区分 | —（判定规则，非测试类型）| [O_boundary.md](./LOG/O_boundary.md) | §4 边界（重点 UTIL/LOG 隔离）|
...
3. **按需加载** 对应子模板
4. **交叉参考** `O_boundary.md` 防止误标 LOG 标签（实际是 UTIL / BIZ / SPECIAL / LINK / UI）
...
> UTIL = 日志**底层采集 SDK + 文件读写 + 网络上报框架**。两者**严格隔离、无职责重叠**。

| 归 LOG | 不归 LOG（归其他模块）|
|------------|--------------------------|
| 玩家行为埋点业务触发规则 | 日志采集 SDK、底层打印工具（归 UTIL）|
| 资产产出消耗审计流水 | 本地文件读写框架（归 UTIL）|
| 全量业务操作日志留痕 | 网络上报底层框架（归 UTIL）|
| 服务监控埋点业务指标 | 日志压缩分片工具（归 UTIL）|
| 客户端崩溃堆栈 + 上下文 | 通用 localStorage 工具（归 UTIL J）|
| 日志分级/分层/生命周期规范 | Redis 缓存（归 UTIL C）|
| 日志完整性/一致性校验 | 通用网络断线重连（归 UTIL B）|
| 埋点字段合规/脱敏 | 加密算法（归 UTIL M）|
| 全链路 TraceID 串联 | 全局崩溃捕获（归 UTIL N）|
| 安全/反作弊日志留痕 | — |
