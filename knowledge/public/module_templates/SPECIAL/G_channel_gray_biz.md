...
> **不测什么**：渠道登录/支付业务对接（归 LINK）、配置灰度字段（归 CONFIG）、底层离线更新框架（归 UTIL F）。
...
### 边界 3：vs UTIL F（离线更新）
- **混淆点**："离线包下载" 看似 G → 实际 UTIL F 测"离线包下载/校验底层"，G 测"离线包损坏/版本不匹配的业务降级"
- **判定规则**：测"离线包底层" → 归 UTIL F；测"离线包异常的业务处理" → 归 SPECIAL G
- **instance**：断点续传 → 归 UTIL F；离线包版本不匹配拦截 → 归 G
